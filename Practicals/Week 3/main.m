%% EGH445 Numerical Methods Computer Lab
close all;
clear;
clc;
...
% Euler Method - ode1
ode1 = sim('MassSpringDamper_Model', 'Solver', 'ode1', ...
'FixedStep', 'deltaT', 'StopTime', 'MSD_1.stop_time');
...
ode4 = sim('MassSpringDamper_Model', 'Solver', 'ode4', ...
'FixedStep', 'deltaT', 'StopTime', 'MSD_1.stop_time');
% Visualisation
figure(1);
...
legend('ODE1', 'ODE4');
title({'Mass-Spring-Damper - Parameter Set 1', ...
sprintf('Integration Step = %0.2fs',deltaT)});
xlabel('Time (sec)');
ylabel('Velocity (m/s)');
...
tstart = tic;
ode4 = sim('MassSpringDamper_Model', 'Solver', 'ode4', ...
'FixedStep', 'MSD_2.time_step', 'StopTime', 'MSD_2.stop_time');
ode4_runtime = toc(tstart);
t_ode4 = ode4.get('tout');
% Variable Time - ode45
tstart = tic;
ode45 = sim('MassSpringDamper_Model', 'Solver', 'ode45', ...
'StopTime', 'MSD_2.stop_time');
ode45_runtime = toc(tstart);
% Variable Time - ode15s
tstart = tic;
ode15s = sim('MassSpringDamper_Model', 'Solver', 'ode15s', ...
'StopTime', 'MSD_2.stop_time');
